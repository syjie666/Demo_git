...
public class SPUM_Prefabs : MonoBehaviour
{
    ...
    public void PlayAnimation(PlayerState PlayState, int index, UnityAction onComplete = null)
    {
        Animator animator = _anim;
        ...
        if (!isMove && !isDebuff)
        {
            ...
            // 启动播放完检测
            StartCoroutine(WaitForAnimation(PlayState, index, onComplete));
        }
    }
    ...
    IEnumerator WaitForAnimation(PlayerState state, int index, UnityAction onComplete = null)
    {
        var clip = StateAnimationPairs[state.ToString()][index];
